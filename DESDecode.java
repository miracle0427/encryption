public class DESDecode {
    ...
    //密文必须是长度为64的倍数的0-1 bit String
    public DESDecode(String ciphertextBitString,String key) {
        assert ciphertextBitString != null && ciphertextBitString.length() > 0 && ciphertextBitString.length() % 64 == 0 && !isContainElseCharacter(ciphertextBitString);
        assert key != null && key.length() == 64 && !isContainElseCharacter(key);
        this.ciphertextBitString = ciphertextBitString;
        this.key = key;
    }
    public String decode(){
        //生成所有的轮密钥
        String[] allKey = new String[16];
        String tempKey = permutedChoice1(this.key);

        for(int i = 0;i < 16;i++){
            tempKey = tempKey.substring(LS[i],28) + tempKey.substring(0,LS[i]) + tempKey.substring(28+LS[i]) + tempKey.substring(28,28+LS[i]);
            //生成真正的轮密钥
            allKey[i] = permutedChoice2(tempKey);
        }
        for(int group = 1;group*64<=this.ciphertextBitString.length();group++){
            String subCipher = this.ciphertextBitString.substring((group - 1) * 64,group*64);
            //对密文进行初始置换(逆置换的逆就是初始置换)
            String temp = InitialPermutation(subCipher);
            String swap = temp.substring(32) + temp.substring(0,32);

            String left = swap.substring(0,32);
            String right = swap.substring(32);

            //16轮解密
            for(int i = 0;i < 16;i++){
                String expansion = expansionPermutation(left);
                String xor1 = XOR(allKey[15-i],expansion);
                String sbox= substitutionChoice(xor1);
                String p = permutation(sbox);
                String x = String.copyValueOf(left.toCharArray());
                left = XOR(right,p);
                right = x;
            }
            this.plainTextBitString += inverseInitialPermutation(left+right);
        }
        for(int group = 1;group * 16 <= this.plainTextBitString.length() ; group++){
            String subBitString = this.plainTextBitString.substring((group-1)*16,group*16);
            this.plainText += (char)Integer.parseInt(subBitString,2);
        }

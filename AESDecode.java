public class AESDecode {
    ...
    //密钥，128位的0-1串
    private String key;
    public AESDecode(String cipherBitString,String key){
        assert cipherBitString != null && cipherBitString.length() > 0 && cipherBitString.length() % 128 == 0 && !isContainElseCharacter(cipherBitString);
        assert key != null && key.length() == 128 && !isContainElseCharacter(key);
        this.cipherBitString = cipherBitString;
        this.key = key;
    }
    public String decode(){
        ...
        for(int group = 1;group * 128 <= this.cipherBitString.length();group++){
            String subBitString = this.cipherBitString.substring((group - 1) * 128,group * 128);
            String roundKey = w[40] + w[41] + w[42] + w[43];
            String inputText = XOR(subBitString,roundKey);

            //开始真正的解密操作，共10轮,最后一轮需要单独处理
            for(int round = 1;round < 10;round++){

                //反向行移位
                inputText = inputText.substring(0,32) + inputText.substring(56,64) + inputText.substring(32,56) +
                        inputText.substring(80,96) + inputText.substring(64,80) + inputText.substring(104) + inputText.substring(96,104);
                ...
                //反向轮密钥加
                inputText = XOR(temp,w[(10 - round) * 4] + w[(10 - round) * 4 + 1] +w[(10 - round) * 4 + 2] +w[(10 - round) * 4 + 3]);
                ...
                inputText = multiplication(inverseMatrix,array);
            }
            //特殊处理最后一轮
            //反向行移位
            inputText = inputText.substring(0,32) + inputText.substring(56,64) + inputText.substring(32,56) +
            ...
            inputText = temp;
                        //轮密钥加
            inputText = XOR(inputText,w[0] + w[1] +w[2] +w[3]);
            this.plainTextBitString += inputText;
        }
    ...
    private String g(String s,int round){
        ...
        String result = "";
        for(int i = 0;i < 4;i++){
            result += substituteBytes1(temp.substring(i * 8,(i + 1) * 8));
        }
        //将结果与论常量异或
     ...
     * S盒置换
     * */
    private String substituteBytes1(String s){
    ...
    private String substituteBytes(String s){
        ...
        int row = Integer.parseInt(s.substring(0,4),2);
        int column = Integer.parseInt(s.substring(4),2);
        int ans = inverseSBox[row][column];
        String temp = Integer.toBinaryString(ans);
        String result = "";
        for(int i = 0;i < 8 - temp.length();i++){
            result += "0";
